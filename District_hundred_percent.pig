...
#Filter the districts which have reached 100percent objective in BPL
filterDistricts = FILTER elements BY TRIM(Project_Objectives_IHHL_BPL) == TRIM(Project_Performance_IHHL_BPL);

#Store the result into hbase table StatewiseDistrictwisePhysicalProgress
STORE filterDistricts INTO 'hbase://Statewise_Hundred_percent_bpl_obj'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('state_data:State_Name,state_data:District_Name,state_data:Project_Objectives_IHHL_BPL,
state_data:Project_Objectives_IHHL_APL,state_data:Project_Objectives_IHHL_TOTAL,state_data:Project_Objectives_SCW,
state_data:Project_Objectives_School_Toilets,state_data:Project_Objectives_Anganwadi_Toilets,state_data:Project_Objectives_RSM,
state_data:Project_Objectives_PC,state_data:Project_Performance_IHHL_BPL,state_data:Project_Performance_IHHL_APL,
state_data:Project_Performance_IHHL_TOTAL,state_data:Project_Performance_SCW,state_data:Project_Performance_School_Toilets,
state_data:Project_Performance_Anganwadi_Toilets,state_data:Project_Performance_RSM,state_data:Project_Performance_PC');

# Store the result into hdfs location from where we can export the results to mysql using sqoop job
STORE filterDistricts into 'hdfs://localhost:9000/bpl100percentOutput' using PigStorage(',');
